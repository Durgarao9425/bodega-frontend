...
const Header: React.FC = () => {
  ...
  // Sync state with URL when location changes (handles back/forward and direct links)
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const q = params.get('search') || '';
    setSearchTerm(q);

    // Extract category from path if on a category page
    const categoryMatch = location.pathname.match(/\/category\/([^/]+)/);
    if (categoryMatch) {
      setSearchCategory(decodeURIComponent(categoryMatch[1]));
    } else {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // If user clears the input after there was a search, reset it
      if (q.length === 0) {
        const params = new URLSearchParams(location.search);
        if (params.get('search')) {
          params.delete('search');
          // Update URL without changing page necessarily
      ...
      // Don't navigate if results are same
      const params = new URLSearchParams(location.search);
      if (params.get('search') === q) return;

      if (searchCategory === 'All') {
        navigate(`/products?search=${encodeURIComponent(q)}`, { replace: true });
      } else {
        navigate(`/category/${encodeURIComponent(searchCategory)}?search=${encodeURIComponent(q)}`, { replace: true });
      }
    }, 500);

    return () => clearTimeout(timer);
  }, [searchTerm, searchCategory, navigate, location.search, location.pathname]);
    ...
    <>
      <header className="bg-[#007F2D] text-white sticky top-0 z-50 w-full shadow-md">
        ...
        <div className="w-full px-3 sm:px-4 md:px-8 py-2.5 flex items-center justify-between gap-3">
          ...
          {/* Hamburger (mobile only) - Order 1 */}
          <button
            className="md:hidden flex flex-col justify-center items-center w-9 h-9 gap-[5px] shrink-0 order-1"
