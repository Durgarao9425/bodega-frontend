    ...
    <div className="bg-[#f8f9fa]">
      <div className="max-w-[1200px] mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-64 shrink-0">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-gray-700 font-bold mb-5 hover:text-primary-500 transition-colors text-sm"
          >
            ←  Back
          </button>

          {/* User avatar + name */}
          <div className="bg-primary-500 rounded-xl p-4 mb-3 flex items-center gap-3 text-white">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-primary-100 truncate">+91 {user?.phone}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            {tabs.map(tab => (
              <button
                ...
                className={`w-full text-left px-4 py-3.5 font-semibold text-sm border-b border-gray-50 last:border-0 flex items-center gap-3 transition-all ${activeTab === tab.id && tab.id !== 'Logout'
          ...
          {activeTab === 'Account Info' && (
            <div>
              <div className="flex items-center justify-between mb-5">
                ...
                {saveMsg && (
                  <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${saveMsg.includes('success') ? 'bg-primary-50 text-primary-600' : 'bg-red-50 text-red-500'}`}>
                    {saveMsg}
                  </span>
                )}
                {!isEditing ? (
                  <button
                    ...
                    className="bg-primary-500 text-white px-5 py-2 rounded-lg font-bold text-sm hover:bg-primary-600 transition-colors"
                  >
                    ✏️ Edit Profile
                  </button>
                ) : (
              ...
              <div className="bg-white p-6 sm:p-8 rounded-xl border border-gray-100 shadow-sm">
                {/* Avatar strip */}
                <div className="w-full bg-primary-50 rounded-xl h-28 mb-6 flex items-center justify-center">
                  <div className="w-20 h-20 bg-primary-500 rounded-2xl flex items-center justify-center text-white font-extrabold text-3xl shadow-md">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    {profileLoading ? (
                      ...
                      <input
                        ...
                        className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 bg-white disabled:bg-gray-50 disabled:text-gray-500 transition"
                      />
                    )}
                  </div>
      ...
      {orders.map(order => (
        <div key={order._id} className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
          <div className="flex justify-between items-center mb-4 border-b border-gray-50 pb-3">
            ...
            <span className="bg-primary-50 text-primary-600 text-xs font-bold px-3 py-1.5 rounded-full border border-primary-100">
          ...
          <div className="space-y-3 mb-4">
            {order.items?.map((item: any, i: number) => (
              <div key={i} className="flex gap-4 items-center">
                ...
                <p className="font-bold text-primary-500 text-sm hidden sm:block">₹{(item.quantity * item.price).toFixed(2)}</p>
              </div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pt-3 border-t border-gray-50 gap-4">
            ...
            <div className="flex items-center gap-4 w-full sm:w-auto justify-between">
              <p className="text-sm font-bold">Total: <span className="font-extrabold text-gray-900 text-lg">₹{order.totalAmount.toFixed(2)}</span></p>
              <button 
                onClick={() => navigate('/track-order', { state: { order } })}
                className="bg-primary-500 text-white px-4 py-2 rounded-lg font-bold text-xs hover:bg-primary-600 transition-all shadow-sm flex items-center gap-2"
              >
                🚚 Track Order
              </button>
            </div>
...
const AddressesTab = () => {
  ...
  if (loading) return <Loader text="Loading your addresses..." />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {addresses.map(addr => (
          <div key={addr._id} className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm relative hover:border-primary-500 transition-colors group">
            <div className="flex items-center justify-between mb-3">
              <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${addr.type === 'Home' ? 'bg-primary-50 text-primary-600' : 'bg-primary-100 text-primary-700'}`}>
                {addr.type || 'Other'}
              </span>
...
const ThemesTab = () => {
  ...
  const previewColors = THEMES[theme];

  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-extrabold text-gray-900 mb-1">App Themes</h1>
        <p className="text-sm text-gray-500">Choose a color scheme that suits your style. All themes are designed to be soft and professional.</p>
      </div>
        ...
        {availableThemes.map((t) => (
          ...
          >
            ...
            <div className="text-left">
              <p className={`font-bold text-sm ${theme === t.name ? 'text-primary-700' : 'text-gray-800'}`}>
                {t.label}
              </p>
              <p className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">
                {theme === t.name ? 'Active' : 'Select Theme'}
              </p>
            </div>
            {theme === t.name && (
              <div className="ml-auto w-6 h-6 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: previewColors.primary500 }}>
