    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      ...
      <div className="max-w-[1200px] mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-64 shrink-0">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-gray-700 font-bold mb-5 hover:text-[#007F2D] transition-colors text-sm"
          >
            ←  Back
          </button>

          {/* User avatar + name */}
          <div className="bg-[#007F2D] rounded-xl p-4 mb-3 flex items-center gap-3 text-white">
          ...
          {activeTab === 'Account Info' && (
            <div>
              <div className="flex items-center justify-between mb-5">
                ...
                {saveMsg && (
                  <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${saveMsg.includes('success') ? 'bg-green-50 text-[#007F2D]' : 'bg-red-50 text-red-500'}`}>
                    {saveMsg}
                  </span>
                )}
                {!isEditing ? (
                  <button
                    ...
                    className="bg-[#007F2D] text-white px-5 py-2 rounded-lg font-bold text-sm hover:bg-[#006e27] transition-colors"
                  >
                    ✏️ Edit Profile
                  </button>
                ) : (
              ...
              <div className="bg-white p-6 sm:p-8 rounded-xl border border-gray-100 shadow-sm">
                ...
                <div className="w-full bg-green-50 rounded-xl h-28 mb-6 flex items-center justify-center">
                  <div className="w-20 h-20 bg-[#007F2D] rounded-2xl flex items-center justify-center text-white font-extrabold text-3xl shadow-md">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    {profileLoading ? (
                      ...
                      <input
                        ...
                        className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#007F2D]/30 focus:border-[#007F2D] bg-white disabled:bg-gray-50 disabled:text-gray-500 transition"
                      />
                    )}
                  </div>
